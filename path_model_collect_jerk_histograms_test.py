...
if not os.path.exists('Gauss_Bsurf.mat'): 
    os.system('wget http://morpho.ipgp.fr/4DEarth/Gauss_Bsurf.mat')
...
theta_grid, phi_grid, components = np.meshgrid(thetas, phis,run_components)
thetaphi_g = np.transpose(np.vstack((theta_grid.flatten(), phi_grid.flatten(), components.flatten() )))
npt = np.shape(thetaphi_g)[0]

def my_calc_par(thetaphi_g):
    print('Calculating  ',thetaphi_g)
