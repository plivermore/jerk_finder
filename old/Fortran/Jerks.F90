...
! jerk_data: 2D array of change in slope and the time of each changepoint, from the thinned Markov chain. (2D array, dimensions: max_number_changepoints, 2).
! jerk_data(:,1) are the times; jerk_data(:,2) are the changes in slope. The parameter max_number_changepoints = (K_MAX-K_MIN+1)*(NSAMPLE -BURN_IN)/THIN.
...
REAL( KIND = dp), intent(out) :: jerk_data( (K_MAX-K_MIN+1)*(NSAMPLE - BURN_IN)/THIN, 2)
